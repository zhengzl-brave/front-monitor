import { IAnyObject, IntegrationError } from 'front-monitor-types';
import { globalVar, HttpCodes, ErrorTypes } from 'front-monitor-shared';
...
export function replaceOld(
  source: IAnyObject, // 需要被重写的对象
  name: string, // 需要被重写对象的key
  replacement: (...args: any[]) => any, // 以原有函数作为参数，执行并重写原函数
  isForced = false, // 是否强制重写（可能原先没有该属性）
): void {
