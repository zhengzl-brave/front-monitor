import { BreadCrumbTypes, BreadCrumbCategory } from 'front-monitor-shared';
import {
  logger,
  validateOption,
  getTimestamp,
...
import { BreadcrumbPushData, InitOptions } from 'front-monitor-types';
...
export class Breadcrumb {
  ...
  push(data: BreadcrumbPushData): void {
    // beforePushBreadcrumb: 每次用户行为事件前会调用
  ...
  // 传入重写的事件类型匹配对应用户行为的整合类型，如 xhr, fetch => http
