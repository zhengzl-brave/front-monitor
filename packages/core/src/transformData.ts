import { BreadCrumbTypes, ErrorTypes, globalVar } from 'front-monitor-shared';
import {
  getLocationHref,
  getTimestamp,
  Severity,
...
import { ReportDataType, MonitorHttp, Replace, ResourceErrorTarget } from 'front-monitor-types';
...
export function httpTransform(data: MonitorHttp): ReportDataType {
  ...
  const name = `${type}--${method}`; // xhr-get
  if (status === 0) {
    // elapsedTime 表示 运行时间
    message =
